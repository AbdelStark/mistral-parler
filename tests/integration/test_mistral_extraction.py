...
def short_transcript():
    ...
    segments = [
        TranscriptSegment(
            id=i, start_s=float(i * 10), end_s=float((i + 1) * 10),
...
def long_transcript():
    ...
    segment = TranscriptSegment(
        id=0, start_s=0.0, end_s=3600.0, text=long_text,
        language="en", speaker_id=None, speaker_confidence=None,
        confidence=0.9, no_speech_prob=0.01, code_switch=False, words=None
    )
    return Transcript(
        text=long_text,
...
class TestSinglePassExtraction:
        ...
        with patch("parler.extraction.extractor.MistralClient") as MockClient:
            ...
            mock_instance.chat.complete.return_value = MagicMock(
                choices=[MagicMock(message=MagicMock(
                    content=json.dumps(make_extraction_response())
                ))]
            )
    ...
    def test_extract_uses_correct_model(self, short_transcript):
        ...
        assert call_kwargs.kwargs.get("model") == "mistral-large-latest" or \
               (call_kwargs.args and call_kwargs.args[0] == "mistral-large-latest") or \
               "mistral-large-latest" in str(call_kwargs)
    ...
    def test_extract_temperature_is_zero(self, short_transcript):
        ...
        assert call_kwargs.kwargs.get("temperature") == 0.0 or \
               "temperature=0" in str(call_kwargs)
        ...
        with patch("parler.extraction.extractor.MistralClient") as MockClient:
            ...
            mock_instance.chat.complete.return_value = MagicMock(
                choices=[MagicMock(message=MagicMock(
                    content=json.dumps(make_extraction_response(
                        decisions=[{
                            "id": "D1", "summary": "Launch in May",
                            "confidence": "high", "language": "fr",
                            "quote": "On lance en mai.", "timestamp_s": 10.0,
...
class TestMultiPassExtraction:
    ...
    def test_long_transcript_triggers_multiple_api_calls(self, long_transcript):
        ...
        with patch("parler.extraction.extractor.MistralClient") as MockClient:
            ...
            extractor = DecisionExtractor(
                api_key="test-key", model="mistral-large-latest",
    ...
    def test_multi_pass_decisions_merged_and_deduplicated(self, long_transcript):
        ...
        call_count = [0]
        decisions_per_pass = [
            [{"id": "D1", "summary": "Launch decision", "confidence": "high",
              "language": "en", "quote": ".", "timestamp_s": 1.0, "speaker": None, "confirmed_by": []}],
