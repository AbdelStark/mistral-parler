...
class TestChunking:
    ...
    def test_long_audio_split_into_multiple_chunks(self):
        ...
        chunk_response.segments = [
            MagicMock(id=0, start=0.0, end=5.0, text="Segment text.",
...
class TestRetryBehaviour:
    ...
    def test_429_after_max_retries_raises_api_error(self, mock_audio_file):
        ...
        with patch("parler.transcription.transcriber.MistralClient") as MockClient, \
            ...
            transcriber = VoxtralTranscriber(api_key="test-key", model="voxtral-v1-5", max_retries=3)
...
class TestCacheIntegration:
    ...
    def test_cached_transcript_skips_api_call(
        ...
        cached_transcript = Transcript(
            ...
            segments=(
                TranscriptSegment(
                    id=0, start_s=0.0, end_s=5.0, text="Cached.", language="fr",
        ...
        with patch("parler.transcription.transcriber.MistralClient") as MockClient:
            mock_instance = MockClient.return_value
            transcriber = VoxtralTranscriber(
                api_key="test-key", model="voxtral-v1-5", cache=cache
            )
            result = transcriber.transcribe(mock_audio_file)
    ...
    def test_different_content_hash_misses_cache(
        ...
        audio_a = AudioFile(
            path=Path("/tmp/a.mp3"), original_path=None, format="mp3",
            duration_s=60.0, sample_rate=44100, channels=2, size_bytes=1000,
            content_hash="aaaa1111aaaa1111",
        )
        audio_b = AudioFile(
            path=Path("/tmp/b.mp3"), original_path=None, format="mp3",
            duration_s=60.0, sample_rate=44100, channels=2, size_bytes=1000,
